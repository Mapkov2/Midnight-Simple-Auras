...
function MSWA_GetDB()
    ...
    if not db.schemaVersion or db.schemaVersion < 3 then
    ...
    if not db.schemaVersion or db.schemaVersion < 4 then
    ...
    if not db.schemaVersion or db.schemaVersion < 5 then
        db._instanceCounter = db._instanceCounter or 0
        db.schemaVersion = 5
    end

    if not db.position then
        db.position = { x = 0, y = -150 }
    end
    ...
    if not db.textPoint then
        db.textPoint = "BOTTOMRIGHT"
    end
